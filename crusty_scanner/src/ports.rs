...
pub async fn scan_ports(concurrency: usize, subdomain: Subdomain) -> Subdomain {
    ...
    // using channels for concurrent streams
    ...
    let output_rx_stream = tokio_stream::wrappers::ReceiverStream::
            new(output_rx);
...
async fn scan_port(mut socket_address: SocketAddr, port: u16) -> Port {
    ...
    let is_open = matches!(
        // tokio::time::timeout takes a time argument and a future and returns 
        // Ok() if the future executes before the specified timeout
