...
impl CveDetailsSpider {
    pub fn new() -> Self {
        ...
        let http_client = Client::builder()
            ...
            .build()
            .expect("Failed building HTTP client in spiders/cvedetails");
...
impl super::Spider for CveDetailsSpider {
    ...
    async fn scrape(&self, url: String) -> Result<(Vec<Self::Item>, Vec<String>), Error> {
        ...
        let http_result = self.http_client.get(url).send().await?.text().await?;
        ...
        let next_pages_links = document
            .select(Attr("id", "pageingb").descendant(Name("a")))
