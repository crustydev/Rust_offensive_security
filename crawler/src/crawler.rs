...
impl Crawler {
    ...
    pub async fn run<T: Send + 'static>(&self, spider::Arc<dyn Spider<Item = T>>) {
        ...
        loop {
            ...
            if new_urls_tx.capacity() == crawling_queue_capacity // new_urls channel empty
                && urls_to_visit_tx.capacity() == crawling_queue_capacity // urls_to_visit channel empty
                && active_spiders.load(Ordering::SeqCst) == 0
            {
                // no work left to be done, exit
                break;
            }
            ...
            tokio_stream::wrappers::ReceiverStream::new(urls_to_visit)
                .for_each_concurrent(concurrency, |queued_url| {
                    ...
                    async {
                        ...
                        if let Some((items, scraped_urls)) = res {
                            for item in items {
